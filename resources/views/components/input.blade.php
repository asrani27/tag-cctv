...
<div class="w-full">
    ...
    <div class="relative">
        ...
        <input
            ...
            {{ $required ? 'required' : '' }}
            {{ $attributes->merge([
                'class' => 'block w-full rounded-lg border text-sm transition-colors duration-150 ' .
                    ($icon ? 'pl-9 ' : 'px-3.5 ') .
                    'py-2.5 ' .
                    ($hasError
                        ? 'border-rose-400 text-rose-900 placeholder-rose-300 focus:border-rose-500 focus:ring-2 focus:ring-rose-200 bg-rose-50/20'
                        : 'border-slate-300 text-slate-900 placeholder-slate-400 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200/60 bg-white')
            ]) }}
        />
    </div>
